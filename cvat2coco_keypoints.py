...
for interImage in interImages:
    ...
    for interObj in interImage.objs:
        ...
        for point in interObj.annos[0].points:
            if point.x < 20 or point.y < 20:  # 一些遮挡点置为0
                point.x = 0
                point.y = 0
        ...
        coco_annotation = CocoAnnotation(segmentation=[], num_keypoints=num_keypoints, area=0., iscrowd=0, keypoints=keypoints, 
                                        image_id=coco_image.id, id=len(coco_annotations), bbox=[], category_id=labels.index(interObj.label))
...
cocoKeypointsJson.export_to_json_file(filename='pallet_26keypoints.json')
